...
public class EmailService {
    ...
    public void sendResetPassword(String to, String username, String resetToken) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            helper.setTo(to);
            helper.setSubject("OTP Verification");
            helper.setText(Const.HTML_OTP_MAIL.formatted(username, resetToken), true);
            mailSender.send(message);
            log.info("Thread: " + Thread.currentThread().getName());
        } catch (MessagingException e) {
            log.info("Failed to send reset password email to {}" + e.getMessage());
        } catch (Exception e) {
            log.info("Unexpected error when sending reset password to {}" + e.getMessage());
        }
    }
}
