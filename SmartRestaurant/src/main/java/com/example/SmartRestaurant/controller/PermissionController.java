...
import com.example.SmartRestaurant.service.permission.PermissionService;
...
@RequestMapping(Const.PREFIX_VERSION + "/permission")
...
public class PermissionController {
    ...
    ResponseEntity<ApiResponse<?>> getAll(@RequestParam(required = false) Long userId) {
        return ResponseEntity.status(HttpStatus.OK).body(new ApiResponse<>(
                200
                , "Success"
                , userId != null ? permissionService.getAllByUserId(userId) : permissionService.getAll()
                , LocalDateTime.now()));
    }
}
